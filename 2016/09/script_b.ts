...
function decode(string: string, indexToStartFrom: number) {
	let iOpeningbracket: number = string.indexOf(openingbracket, indexToStartFrom);
	
	if(iOpeningbracket !== -1) {
		let iMultiplier: number = string.indexOf(multiplier, iOpeningbracket);
		let iClosingbracket: number = string.indexOf(closingbracket, iOpeningbracket);

		let nrOfCharsToRepeat: number = parseInt(string.substr(iOpeningbracket+1, iMultiplier))
		let repeats: number = parseInt(string.substr(iMultiplier+1, iClosingbracket))
		
		let charsToRepeat: string = string.substr(iClosingbracket+1, nrOfCharsToRepeat)

		let charsRepeated: string = "";
		let tempRepeats: number = repeats;
		while(tempRepeats > 0) {
			charsRepeated += charsToRepeat;
			tempRepeats--;
		}

		let newVal: string = string.substr(0, iOpeningbracket) + charsRepeated + string.substr(iClosingbracket+1+charsToRepeat.length)
		let newIndexToStartFrom: number = iOpeningbracket + (charsToRepeat.length * repeats);

		return [newVal, newIndexToStartFrom]
	}
	return [string, undefined];
}


// decoded = decode(formatted, 0);
// while(decoded[1]) {
// 	decoded = decode(decoded[0], decoded[1]);
// }

// console.log("decoded length is", decoded[0].length)
let totLength: number = 0;
function check(string, startAt) {
	let indexToStartFrom = startAt
	let iOpeningbracket: number = string.indexOf(openingbracket, indexToStartFrom);
	
	if(iOpeningbracket !== -1) {
		let lengthBeforeOpeningbracket: number = string.substr(indexToStartFrom, iOpeningbracket).length
		...
		let charsToRepeat: string = string.substr(iClosingbracket+1, nrOfCharsToRepeat)
		let openingBrackets = charsToRepeat.match(/\(/g);
		let nrOfOpeningBrackets = 0;
		if(openingBrackets) { nrOfOpeningBrackets = openingBrackets.length; }

		let length = nrOfCharsToRepeat * (repeats-1)
		let endI = iClosingbracket + nrOfCharsToRepeat

		return {
			charsToRepeat: charsToRepeat,
			repeats: repeats,
			length: nrOfCharsToRepeat * repeats,
			lengthBefore: lengthBeforeOpeningbracket,
			lastIndex: endI+1,
			nrOfOpeningBrackets: nrOfOpeningBrackets
		}
	} else {
		let charsToRepeat = string.substr(indexToStartFrom);
		let charsToRepeatLength = charsToRepeat.length;
		return {
			charsToRepeat: charsToRepeat,
			repeats: 1,
			length: charsToRepeatLength,
			lengthBefore: 0,
			lastIndex: indexToStartFrom + charsToRepeatLength
		}
	}
}

// debugger
// let checked = check(formatted, 0);
// let totalLength = 0;
// let repeater = 1
// if(checked.charsToRepeat.match(/([1-9]){1,}x([1-9]{1,})/g)) {
// 	totalLength += checked.lengthBefore
// 	repeater = checked.repeats
// 	let checked2 = check(checked.charsToRepeat);

// 	if(checked2.charsToRepeat.match(/([1-9]){1,}x([1-9]{1,})/g)) {
// 		repeater *= checked2.repeats
// 		let checked3 = check(checked2.charsToRepeat);

// 		if(checked3.charsToRepeat.match(/([1-9]){1,}x([1-9]{1,})/g)) {
// 			repeater *= checked3.repeats
// 			let checked4 = check(checked3.charsToRepeat);

// 		} else {
// 			totalLength = totalLength + (repeater * checked2.charsToRepeat.length)
// 		}

// 	} else {
// 		totalLength = totalLength + (repeater * checked2.repeats * checked2.charsToRepeat.length)
// 	}
// }
...
let totalLength = 0
let lastIndex = 0;
let nrOfOpeningBrackets = 1;
// formatted = (25x3)(3x3)ABC(2x3)XY(5x2)PQRSTX(18x9)(3x2)TWO(5x7)SEVEN
while(lastIndex < formatted.length) {
	checked = check(formatted, lastIndex); // checked = (3x3)ABC(2x3)XY(5x2)PQRST
	repeater = checked.repeats
	totalLength += checked.lengthBefore;
	lastIndex = checked.lastIndex
	let tempChecked = checked.charsToRepeat
	// while(nrOfOpeningBrackets > 0) {
	// 	while(tempChecked.match(/([1-9]){1,}x([1-9]{1,})/g)) {
	// 		tempChecked = check(checked.charsToRepeat, 0);
	// 		totalLength += tempChecked.lengthBefore
	// 		repeater *= tempChecked.repeats
	// 	}
	// 	totalLength = totalLength + (repeater * checked.charsToRepeat.length)
	// }

	while(nrOfOpeningBrackets > 0 && tempChecked.match(/([1-9]){1,}x([1-9]{1,})/g)) {
		tempChecked = check(checked.charsToRepeat, 0);
		if(nrOfOpeningBrackets === undefined) { nrOfOpeningBrackets = tempChecked.nrOfOpeningBrackets } // nrOfOpeningBrackets = 3
		totalLength += tempChecked.lengthBefore
		repeater *= tempChecked.repeats

		if(!tempChecked.match(/([1-9]){1,}x([1-9]{1,})/g)) {
			nrOfOpeningBrackets--
			totalLength = totalLength + (repeater * checked.charsToRepeat.length)
			repeater = 0
		}
...
// let regex = /\(([1-9])x([1-9]){0,}\)/g
// let test = formatted.match(regex)
// let fomattedNoBrakcets = formatted.replace(regex, "");

// let tester = formatted.split(/\(|\)|\s/)

// function splitBrackets(string) {
// 	let [chars, repeats] = string.split("x")
// 	return {chars: chars, repeats: repeats};
// }
...
// for(let part of tester) {
// 	if(part === "") {
// 		tester.splice(tester.indexOf(part), 1)
// 	}
// }

// for(let i in tester) {
// 	// while(part.match(/([1-9]){1,}x([1-9]{1,})/g) === null) {
// 	// 	let splitted = splitBrackets(part);
// 	// 	let chars = splitted[0];
// 	// 	let repeats = splitted[1];
// 	// 	let countLength = chars * (repeats-1)
// 	// 	theLength += countLength;
// 	// }

// 	debugger
// 	if(tester[i].match(/([1-9]){1,}x([1-9]{1,})/g)) {
// 		let splitted = splitBrackets(tester[i]);
// 		let chars = splitted.chars
// 		if(tester[i+1].match(/([1-9]){1,}x([1-9]{1,})/g)) {

// 		}

// 		// theLength += splitBrackets(part);
// 		while(part.match(/([1-9]){1,}x([1-9]{1,})/g) === null) {

// 		}
// 		console.log("brackets");
// 	} else {
// 		theLength += part.length;
