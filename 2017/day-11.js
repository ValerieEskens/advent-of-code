...
let input = "ne,ne,ne" // 3 steps away.
input = "ne,ne,sw,sw" // 0 steps away (back where you started).
input = "ne,ne,s,s" // 2 steps away (se,se).
// input = "se,sw,se,sw,sw" // 3 steps away (s,s,sw).
...
inputArr.forEach(direction => {
  ...
  move(direction)
  stepsAway(position.x, position.y, position.z)
  if(position.x - position.y > farestPos.x - farestPos.y) {
    farestPos = {
      x: position.x,
      y: position.y
    }
    console.log('farestPos', farestPos.x - farestPos.y)
  }
})
...
function stepsAway(x, y, z) {
  let max = Math.max(x, y, z)
  let stepsAway
  // switch (max) {
  //   case x: stepsAway = Math.abs(y-x) + Math.abs(z-x)
  //     break;
  //   case y: stepsAway = Math.abs(x-y) + Math.abs(z-y)
  //     break;
  //   case z: stepsAway = Math.abs(x-z) + Math.abs(y-z)
  //     break;
  //   default:
  //     break;
  // }
  ...
  if(x === 0) {
    if(y === 0) {
      // x y
      console.log(`${x}, ${y}, ${z}: ${Math.abs(z)} steps away`)
    } else if(z === 0) {
      // x z
      console.log(`${x}, ${y}, ${z}: ${Math.abs(y)} steps away`)
    } else {
      // x
      console.log(`${x}, ${y}, ${z}: ${(Math.abs(y)+Math.abs(z))} steps away`)
    }
  } else if(y === 0) {
    if(z === 0) {
      // y z
      console.log(`${x}, ${y}, ${z}: ${Math.abs(x)} steps away`)
    } else {
      // y
      console.log(`${x}, ${y}, ${z}: ${(Math.abs(x)+Math.abs(z))} steps away`)
    }
  } else if (z === 0) {
    // z
    console.log(`${x}, ${y}, ${z}: ${(Math.abs(x)+Math.abs(y))} steps away`)
  } else {
    console.log(`${x}, ${y}, ${z}: ${(x-z)+(y-z)} steps away`)
  }
}