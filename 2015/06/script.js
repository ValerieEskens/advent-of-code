...
(function() {
  ...
  splitCoordinate = function(coordinate) {
    var ref, x, y;
    ref = coordinate.split(","), x = ref[0], y = ref[1];
    return {
      x: x,
      y: y
    };
  ...
  setLight = function(action, x, y) {
    if (lights[x] === void 0) {
      lights[x] = {};
    }
    if (lights[x][y] === void 0) {
      lights[x][y] = false;
    }
    switch (action) {
      case "+":
        return lights[x][y] = true;
      case "-":
        return lights[x][y] = false;
      case "*":
        return lights[x][y] = !lights[x][y];
    }
  };

  for (i = j = 0, len = splittedInstructions.length; j < len; i = ++j) {
    instruction = splittedInstructions[i];
    splittedInstruction = instruction.split(" ");
    instructions[i] = {};
    if (splittedInstruction[0] === "turn") {
      if (splittedInstruction[1] === "on") {
        instructions[i].action = "+";
      }
      if (splittedInstruction[1] === "off") {
        instructions[i].action = "-";
      }
