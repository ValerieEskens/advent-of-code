let codeString: string = `hqcfqwydw-fbqijys-whqii-huiuqhsx-660[qhiwf]
...
// codeString = `qzmt-zixmtkozy-ivhz-343[zmtiq]`;

let roomsString: Array<string> = codeString.split("\n");
let rooms: Array<Object> = orderRooms(roomsString)
let sumSectorIds: number = 0;
let checkedRooms: Array<Object> = []

function countChars(name: string): Array<Array<string>> {
  let chars: Array<Array<string>> = [];
  let nameNoDashes: string = name.replace(/-/g, "");
  while(nameNoDashes != "") {
    let char: string = nameNoDashes[0];
    let charRegexp = new RegExp(char, "g");
    let occurences: number = nameNoDashes.match(charRegexp).length
    if(!chars[occurences]) { chars[occurences] = [] }
    chars[occurences].push(char);
    nameNoDashes = nameNoDashes.replace(charRegexp, "");
  }
  return chars;
}

function orderRooms(roomsString: Array<string>): Array<Object> {
  let rooms: Array<Object> = [];
  for(let room of roomsString) {
    let [nameAndSectorId, checksum] = room.replace("]", "").split("[");
    let sectorId = nameAndSectorId.substr(nameAndSectorId.length-3, nameAndSectorId.length);
    let name = nameAndSectorId.substr(0, nameAndSectorId.length-4)
    rooms.push({
      name: name,
      sectorId: sectorId,
      checksum: checksum
...
function checkRoom(room: Object): Object {
  let charCounts: Array<Array<string>> = countChars(room.name);
  let i = charCounts.length-1
  for(let digit of room.checksum) {
    while(charCounts[i] === undefined || charCounts[i].length === 0) {
      i--;
    }
    if(charCounts[i].indexOf(digit) === -1) {
      return {};
    } else {
      charCounts[i].splice(charCounts[i].indexOf(digit), 1);
    }
  }
  return room;
}
...
for(let room of checkedRooms) {
  ...
  for(let char of room.name) {
    if(char === "-") {
      ...
      let shiftedCharAscii = charAscii + shift;
      if(shiftedCharAscii > 122) {
        shiftedCharAscii = shiftedCharAscii - 122 + 96
      }
      let shiftedChar: string = String.fromCharCode(shiftedCharAscii);
