...
function decode(string, startI, alreadyRepeats) {
	let iOpeningbracket: number = string.indexOf(openingbracket, startI);
	let nextI;

	if(iOpeningbracket !== -1) {
		let charsBefore = iOpeningbracket - startI
		if(charsBefore !== 0) {
			parts.push({ repeats: alreadyRepeats, chars: string.substr(startI, charsBefore) })
		}
		...
		let nrOfChars: number = parseInt(string.substr(iOpeningbracket+1, iMultiplier));
		let repeats: number = parseInt(string.substr(iMultiplier+1, iClosingbracket)) * alreadyRepeats;

		let chars = string.substr(iClosingbracket+1, nrOfChars);

		parts.push({ repeats: repeats , chars: chars })
		nextI = iClosingbracket + nrOfChars + 1
	} else {
		parts.push({ repeats: alreadyRepeats, chars: string.substr(startI) })
		nextI = string.length + 1
	}
...
function goThrough() {
	for(let i in parts) {
		let part = parts[i]
		if(part.chars.match(/\(([0-9]){1,}x([0-9]{1,})\)/g)) {
			let nextI = 0
			while(nextI < part.chars.length) {
				nextI = decode(part.chars, nextI, part.repeats)
			}
			parts[i] = undefined
		} else {
			totalLength = totalLength + (part.repeats * part.chars.length);
			parts[i] = undefined
		}
	}

	parts = parts.filter(function(part){ return part !== undefined });
}

while(nextI < formatted.length) {
	nextI = decode(formatted, nextI, 1)
}

for(let part of parts) {
	totalLength = totalLength + part.repeats * part.chars.length
}
console.log("totalLength part 1", totalLength)
...
console.log("totalLength part 2", totalLength)