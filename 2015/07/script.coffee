        ...
        do ->
            if cir isnt false
                ...
                if not isNaN(c[0]*1)
                    ...
                    if c[0].indexOf('AND') isnt -1
                        [val1, val2] = c[0].split('AND')
                        validatedVal1 = isValueValid(val1)
                        validatedVal2 = isValueValid(val2)
                        if validatedVal1? and validatedVal2?
                            performAnd validatedVal1, validatedVal2, c[1]
                            remove cir
                    else if c[0].indexOf('OR') isnt -1
                        [val1, val2] = c[0].split('OR')
                        validatedVal1 = isValueValid(val1)
                        validatedVal2 = isValueValid(val2)
                        if validatedVal1? and validatedVal2?
                            performOr validatedVal1, validatedVal2, c[1]
                            remove cir
                    else if c[0].indexOf('NOT') isnt -1
                        val = c[0].replace('NOT', '')
                        validatedVal1 = isValueValid(val)
                        if validatedVal1?
                            performNot validatedVal1, c[1]
                            remove cir
                    else if c[0].indexOf('LSHIFT') isnt -1
                        [val1, val2] = c[0].split('LSHIFT')
                        validatedVal1 = isValueValid(val1)
                        validatedVal2 = isValueValid(val2)
                        if validatedVal1? and validatedVal2?
                            performLshift validatedVal1, validatedVal2, c[1]
                            remove cir
                    else if c[0].indexOf('RSHIFT') isnt -1
                        [val1, val2] = c[0].split('RSHIFT')
                        validatedVal1 = isValueValid(val1)
                        validatedVal2 = isValueValid(val2)
                        if validatedVal1? and validatedVal2?
...
performAnd = (val1, val2, res) ->
    firstNr = toBits val1
    secondNr = toBits val2

    result = ""
    for index, bit of firstNr
        if firstNr[index]? and secondNr[index]?
            d = firstNr[index] and secondNr[index]
            result += d

    known[res] = toInt result
    console.log res, known[res]

performOr = (val1, val2, res) ->
    firstNr = toBits val1
    secondNr = toBits val2

    result = ""
    for index, bit of firstNr
        d = firstNr[index] or secondNr[index]
        result += d
...
performNot = (val, res) ->
    result = ""
    for index, bit of toBits(val)
        if bit is 1 then result += 0
        if bit is 0 then result += 1
    known[res] = toInt(result)
    console.log res, known[res]
performRshift = (val1, val2, res) ->
    known[res] = val1 >> val2

performLshift = (val1, val2, res) ->
    known[res] = val1 << val2
