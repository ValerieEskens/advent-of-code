instructionsTxt = "turn on 887,9 through 959,629
...
splittedInstructions = instructionsTxt.split(/\s(?=turn|toggle)/) 
...
setLight = (action, x, y) ->
	if lights[x] is undefined
		lights[x] = {}
	if lights[x][y] is undefined
		lights[x][y] = false
		
	switch action
		when "+" then lights[x][y] = true
		when "-" then lights[x][y] = false
		when "*" then lights[x][y] = !lights[x][y]

for instruction, i in splittedInstructions
	splittedInstruction = instruction.split(" ")
	instructions[i] = {}
	if splittedInstruction[0] is "turn"
		if splittedInstruction[1] is "on"
			instructions[i].action = "+"
		if splittedInstruction[1] is "off"
			instructions[i].action = "-"
		instructions[i].startCo = splitCoordinate splittedInstruction[2]
		instructions[i].stopCo = splitCoordinate splittedInstruction[4]
	else
		instructions[i].action = "*"
		instructions[i].startCo = splitCoordinate splittedInstruction[1]
		instructions[i].stopCo = splitCoordinate splittedInstruction[3]

for i, instruction of instructions
	coor = {}
	coor.x = instruction.startCo.x
	coor.y = instruction.startCo.y
	
	while parseInt(coor.x) <= parseInt(instruction.stopCo.x)
		while parseInt(coor.y) <= parseInt(instruction.stopCo.y)
			setLight instruction.action, coor.x, coor.y
			coor.y++
		coor.y = instruction.startCo.y
		coor.x++
			
lightCounter = 0
for i, lightRow of lights
	for t, light of lightRow
		if light is true
			lightCounter++

document.getElementById("part2").innerHTML = "lightCounter #{lightCounter}"