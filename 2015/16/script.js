...
for(aunt in input) {
  matches = 0
  for(feature in input[aunt]) {
    if(feature === "cats" || feature === "trees") {
      if(input[aunt][feature] >= match[feature]){
        matches++
      }
    } else if(feature === "pomeranians" || feature === "goldfish") {
      if(input[aunt][feature] <= match[feature]){
        matches++
      }
    } else if(input[aunt][feature] === match[feature]){
      console.log('aunt', input[aunt])
      matches++
    }
  }
  if(matches === 3) {
    console.log('3 MATCHES', aunt, input[aunt])
  } 
}